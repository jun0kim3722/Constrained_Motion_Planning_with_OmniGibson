...
def get_grasp_position_for_open(robot, target_obj, should_open, relevant_joint=None, num_waypoints="default", offset=0.06):
    """
    ...
    Args:
      ...
      offset: distance from eef joint to gripper
    ...
    relevant_joints = [relevant_joints[i] for i in th.randperm(len(relevant_joints))]
    selected_joint = None
    for joint in relevant_joints:
        ...
        if (should_open and openness_fraction < joint.upper_limit/4) or (
...
def grasp_position_for_open_on_prismatic_joint(robot, target_obj, relevant_joint, should_open, num_waypoints="default", offset=0.06):
    """
    ...
    points_along_push_axis = (
        th.cat((canonical_push_axis.unsqueeze(0), -canonical_push_axis.unsqueeze(0))) * 
        bbox_extent_in_link_frame[push_axis_idx] / 2
    )
    (
    ...
    x_axis_idx, y_axis_idx, _ = tuple(sorted(all_axes))
    ...
    grasp_pose_in_world_frame = (
        ...
        T.quat_multiply(th.tensor([0.707, 0.0, 0.0, 0.707]), grasp_pose[1])
    )
    ...
    offset_grasp_pose_in_world_frame = (
        ...
        T.quat_multiply(th.tensor([0.707, 0.0, 0.0, 0.707]), offset_grasp_pose[1])
    )
    ...
    target_hand_pose_in_world_frame = (
        ...
        T.quat_multiply(th.tensor([0.707, 0.0, 0.0, 0.707]), target_hand_pose[1])
    )
...
def interpolate_waypoints(start_pose, end_pose, num_waypoints="default"):
    ...
    if num_waypoints == "default":
        num_waypoints = max([2, int(travel_distance / 0.01) + 1])

    fracs = th.linspace(0, 1, num_waypoints)
    diff_vec = end_pose[0] - start_pos
    pos_waypoints = start_pos + fracs.unsqueeze(1) * diff_vec 

    # Also interpolate the rotations
    orn_waypoints = T.quat_slerp(start_orn.unsqueeze(0), end_pose[1].unsqueeze(0), fracs.unsqueeze(1))
    return [waypoint for waypoint in zip(pos_waypoints, orn_waypoints)]


def grasp_position_for_open_on_revolute_joint(robot, target_obj, relevant_joint, should_open, offset=0.06):
    """
...
def _get_closest_point_to_point_in_world_frame(
    ...
    """
    vectors_in_world = th.cat(
        [
            T.pose_transform(*arbitrary_frame_to_world_frame, vector, th.tensor([0, 0, 0, 1]))[0].unsqueeze(0)
