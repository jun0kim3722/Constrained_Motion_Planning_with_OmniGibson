...
def quat2axisangle(quat):
    ...
    # clip quaternion
    qw = th.clip(quat[:, 3], -1.0, 1.0)
    # quat[:, 3] = qw.clone()
    quat = th.cat((quat[:, :3], qw.unsqueeze(-1)), dim=-1)
    ...
    result = (quat[idx, :3] * 2.0 * th.acos(quat[idx, 3]).unsqueeze(-1)) / den[idx].unsqueeze(-1)
    ret = ret.index_copy(0, idx, result)
