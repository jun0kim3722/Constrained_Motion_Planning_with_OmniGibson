...
class ActionPlan():
    ...
    def cut(self, planner, target_obj, const_dict, offset_joints=None, **kwargs):
        ...
        extra_rot = T.euler2quat(th.tensor([0,0.7,np.arctan2(direction[1], direction[0])]))

        if (T.quat2euler(self.in_hand.get_position_orientation()[1])[:2] < 0.2).all():
            ...
            offset = th.tensor([-0.10, 0.00, 0.1])
            obj_trans = center_loc + T.quat_apply(extra_rot, offset)
        ...
        with PlanningContext(self.env, self.robot, self.collision_joints, self.in_hand) as context:
            breakpoint()
            context.set_arm_and_detect_collision(ready_joints, True)
        ...
        disabled_collision = {target_obj.prim_path : obj_prim_path}
        ...
        # get cut joints
        for cut_height in np.arange(0.14, 0.05, -0.01):
            ...
            cut_end_joints = self.ik_solver.solve_newcoord(*cut_end_pos)
