...
class ArmConstraint(ob.Constraint):
    ...
    def __init__(self, context, trans_const, rot_const, num_const):
        super(ArmConstraint, self).__init__(6, num_const) # ambient, constraint;!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if self.trans_const_ is not None:
            self.trans_mask_ = np.isfinite(self.trans_const_)
        else:
            self.trans_mask_ = np.full(3, False)

        if self.rot_const_ is not None:
            self.rot_mask_ = np.isfinite(self.rot_const_)
        else:
            self.rot_mask_ = np.full(3, False)

    def function(self, x, out):
        trans, rot = self.fk_solver.get_link_poses_euler(x, [self.eef_name])[self.eef_name]
        ...
        if self.rot_const_ is not None:
            rot_diff = (self.rot_const_ - rot)[self.rot_mask_]
        else:
            rot_diff = []

        out[0:2] = np.concatenate((trans_diff, rot_diff))

    def jacobian(self, x, out):
        trans, rot = self.fk_solver.get_link_poses_euler(x, [self.eef_name])[self.eef_name]

        pos_mask = np.concatenate((self.trans_mask_, self.rot_mask_))
        for j in range(6):
            new_joints = x.copy()
            new_joints[j] += 1e-6
            tran_p, rot_p = self.fk_solver.get_link_poses_euler(new_joints, [self.eef_name])[self.eef_name]

            out[:, j] = (np.concatenate((tran_p - trans, rot_p - rot))  / 1e-6)[pos_mask]
...
class ArmProjection(ob.ProjectionEvaluator):
    ...
    def __init__(self, space, context, trans_const, rot_const):
        ...
        if self.trans_const_ is not None:
            self.pos_mask = np.isfinite(self.trans_const_)

        if self.rot_const_ is not None:
            self.rot_mask = np.isfinite(self.rot_const_)

    def getDimension(self):
        return 6

    def project(self, state, projection):
        trans, rot = self.fk_solver.get_link_poses_euler([state[0], state[1], state[2], state[3], state[4], state[5]], [self.eef_name])[self.eef_name]
...
class ArmCcontrainedPlanner():
    def __init__(self, context, trans_const, rot_const, num_const, tolerance,
        ...
        # Create constraint
        self.constraint = ArmConstraint(context, trans_const, rot_const, num_const)
